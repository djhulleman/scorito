...
- world_cup_exact_score_forecaster.py: scoreline-specific model on top of the
  Elo/Poisson baseline; the Big strategy update's
  DecisionTreeClassifier(max_depth=5, min_samples_leaf=1) and scoreline
  guardrails are retrained with date-safe snapshots.
...
- Outcome/bracket model candidates:
  ...
  * Exact-score tree model:
    the earlier in-sample audit reached 80.0% outcome, 55.0% exact, and 77.5%
    within one goal. The app now reports honest walk-forward metrics instead.
